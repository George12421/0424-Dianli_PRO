...
from common_utils import create_env, create_gym_env, MetricsCallback, get_a2c_training_params
from config import *
import torch.nn as nn


def train_a2c(use_tensorboard=False):
    """Train an A2C (Actor-Critic) agent on the grid2op environment."""
    # Create environment
    ...
    try:
        ...
        # Get A2C-specific training parameters
        a2c_params = get_a2c_training_params()
        
        # Create A2C model with highly optimized parameters for speed
        model = A2C(
            policy="MlpPolicy",
            env=env_gym,
        ...
        metrics_callback = MetricsCallback(model_name=A2C_MODEL_NAME)
        
        # Create progress bar
        pbar = tqdm(total=ITERATIONS, desc="Training A2C", position=0, leave=True)
        ...
        for i in range(ITERATIONS):
            ...
            if (i + 1) % (SAVE_EVERY_STEPS * 10) == 0:  # Save much less frequently
                save_path = os.path.join(SAVE_PATH, f"{A2C_MODEL_NAME}_{i+1}")
                model.save(save_path)
        
        pbar.close()
        
        # Save final model
        final_save_path = os.path.join(SAVE_PATH, f"{A2C_MODEL_NAME}_final")
        model.save(final_save_path)
        
        # Save final metrics
        final_metrics_path = os.path.join(SAVE_PATH, f"{A2C_MODEL_NAME}_final_metrics.json")
        metrics_data = {
...
if __name__ == "__main__":
    ...
    model = train_a2c(use_tensorboard=False)
